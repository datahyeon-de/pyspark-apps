...
def for_each_batch_func(df: DataFrame, epoch_id):
    ...
    print("select from_json_df's person_info.*")
    person_info_df = from_json_df.select('person_info.*')
    person_info_df.show(truncate=False)

    print("======================================================")

    print("selectExpr person_info's address.*")
    sep_address_df = person_info_df.selectExpr('name',
                 'address.*',
                 'CAST(age AS INT) AS age',
                 'hobbies')
    sep_address_df.show(truncate=False)

    print("======================================================")

    print("explode sep_address_df's hobbies[]")
    explode_hobbies_df = sep_address_df.select(
        'name',
        'country',
        'city',
        'age',
    ...
    explode_hobbies_df.show(truncate=False)

    df.unpersist()
    print(f'====================================== epoch_id: {epoch_id} end ======================================')


app_name = 'debug_json_with_list'
spark = SparkSession \
        .builder \
        .appName(app_name) \
        .getOrCreate()
